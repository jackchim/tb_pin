...
class Segment {
	...
	function segment($str){
		include_once APPPATH.'libraries/segment/pscws4.class.php';
		$cws = new PSCWS4();
		$cws->set_charset('utf8');
		$cws->set_dict(APPPATH.'libraries/segment/etc/dict.utf8.xdb');
		$cws->set_rule(APPPATH.'libraries/segment/etc/rules.utf8.ini');
		$cws->set_ignore(true);
		$cws->set_multi(1);
		...
		while ($some = $cws->get_result())
		{
			...
			{
				$target .= ' '.$this->convert_to_py(strtolower($word['word']));
			}
		}
		$cws->close();
		return $target;
	...
	function convert_to_py($str){
		return $this->getFullSpell(iconv('UTF-8', 'GB2312', $str));
	}

	/*
	 function convert($_String, $_Code='gb2312',$isInitial=false)
	 {
		//$str = preg_replace("/\s+/", '|', $_String);
		//$str = preg_replace("/[,|\.]+/", '|', $str);
		//$str = preg_replace("/[，|。]+/", '|', $str);
		$str = preg_replace("/[^\x{4e00}-\x{9fa5}]/iu",'|',$_String);

		$source =  explode('|',$str);
		foreach ($source as $str){
		$target .= ' '.$this->_convert($str,1);
		}
		return $target;
