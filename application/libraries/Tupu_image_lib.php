...
class Tupu_image_lib {
	...
	function clear()
	{
		$props = array('create_thumb','thumb_marker','source_folder', 'dest_folder', 'source_image', 'full_src_path', 'full_dst_path', 'new_image', 'image_type', 'size_str', 'quality', 'orig_width', 'orig_height', 'rotation_angle', 'x_axis', 'y_axis', 'create_fnc', 'copy_fnc', 'wm_overlay_path', 'wm_use_truetype', 'dynamic_output', 'wm_font_size', 'wm_text', 'wm_vrt_alignment', 'wm_hor_alignment', 'wm_padding', 'wm_hor_offset', 'wm_vrt_offset', 'wm_font_color', 'wm_use_drop_shadow', 'wm_shadow_color', 'wm_shadow_distance', 'wm_opacity');
	...
	// --------------------------------------------------------------------
	 ...
	 function create_thumb($source,$type,$width=0,$height=0){
		$this->clear();
		...
		$config['thumb_marker'] = '_'.$type;
		$this->initialize($config);
		return $this->resize();
	}
	
	
	function crop_square($source,$horw){
		$this->clear();
		$config['image_library'] = 'gd2';
		$config['master_dim'] = 'auto';
		$config['source_image'] = $source;
		...
		$config['create_thumb'] = TRUE;
		$config['maintain_ratio'] = TRUE;
		$config['thumb_marker'] = '_square';
		$config['quality'] = 100;
		$config['width'] = $horw;
		$config['height'] = $horw;
		$this->initialize($config);
		return $this->resize_auto_choose($horw);
	}
	
	
	function combin_image($x,$y,$bgpath,$path){
		$this->clear();
		$config = array();
		$config['image_library'] = 'gd2';
		$config['wm_type'] = 'overlay';
		$config['source_image'] = $bgpath;
		$config['wm_overlay_path'] = $path;
		$config['dynamic_output'] = false;
		$config['padding'] = 0;
	...
	function resize_auto_choose($horw)
	{
		...
		$resize_result = $this->$protocol('resize');
		...
		if($resize_result){
			...
			if($this->master_dim == 'height'){
				$config['x_axis'] = ($this->width/2)-($horw/2);
				$config['y_axis'] = 0;
				//$config['height'] = $horw;
				...
				$config['y_axis'] = ($this->height/2)-($horw/2);
			}*/
			$this->source_image = $this->full_dst_path;
			$this->full_src_path = $this->full_dst_path;
			if($this->master_dim == 'height'){
				$this->x_axis = ($this->width/2)-($horw/2);
				$this->y_axis = 0;
				//$config['height'] = $horw;
				//$config['master_dim'] = 'height';
			}elseif ($this->master_dim == 'width'){
				...
				//$config['width'] = $horw;
				//$config['master_dim'] = 'width';
				$this->y_axis = ($this->height/2)-($horw/2);
			}
			$this->width = $horw;
			$this->height = $horw;
			//$this->initialize($config);
